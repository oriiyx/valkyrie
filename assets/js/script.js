  ...
  if ($('#add-more').length) {
    ...
    $('#add-more').on('click', function(event){
      ...
      for (i = 0; i < entries.length; i++){
        if(i == entry_length){
          let new_id = i + 1;
          add_entry = '<div class="form-row website-url-entry-wrap mt-3"><div class="col-sm-5"><input type="url" name="website-url[]" class="form-control website-url-entry " id="website-url-'+new_id+'" data-entry="id-'+new_id+'" placeholder="Enter website url"></div><div class="col-sm-5"><input type="text" name="website-name[]" class="form-control website-name-entry" id="website-name-0" placeholder="Enter website name"></div><div class="col-sm-2"><button type="button" class="btn btn-danger remove-me" data-remove="id-'+new_id+'">Remove</button></div></div>';
        }
      }
      add_entry = $(add_entry);
